...
(async function () {
    ...
    async function searchEpisodes(itemInfoMap) {
        ...
        // 手动匹配生成的季度缓存优先，避免后续自动搜索再次选回普通季度。
        const currentPriority = (Array.isArray(apiPriority) && apiPriority[0] === 'custom'
            && apiConfigs.custom.enabled && apiConfigs.custom.prefix?.trim()) ? 'custom' : 'official';
        const selectedApiConfig = (apiConfigs[currentPriority].enabled && apiConfigs[currentPriority].prefix?.trim())
            ...
            : apiConfigs.custom;
        const animaRes = await lsSeasonSearchEpisodes(_season_key, episode, selectedApiConfig.prefix, animeName);
        ...
        for (const apiKey of apiPriority) {
            ...
            if (searchAnimaInfo && searchAnimaInfo.animes.length > 0) {
                searchAnimaInfo.animes = prioritizeSeasonCandidates(animeName, searchAnimaInfo.animes);
    ...
    async function getEpisodeInfo(is_auto = true) {
        ...
        // 手动匹配记录拥有绝对最高优先级：季度校验/缓存清洗/上下集推理均不得覆盖。
        // 双键容错：_episode_key 由 Emby item 信息确定性推导（季+集），itemId 键兼容旧记录。
        try {
            const manualKeys = [];
            ...
            for (const manualKey of manualKeys) {
                const manualStr = window.localStorage.getItem(manualKey);
                if (!manualStr) { continue; }
                const m = JSON.parse(manualStr);
                if (m && m.episodeId) {
                    console.log('[手动匹配] 命中持久化手动选择，直接使用:', m.animeTitle, '-', m.episodeTitle);
                    return m;
                }
            }
        } catch (e) {
            console.warn('[手动匹配] 读取持久化记录失败:', e);
        }

        // 单集缓存优先于上下集推理，并按启用的 API 组合隔离。
        ...
        if (is_auto && window.localStorage.getItem(unique_episode_key)) {
            ...
            // [PITFALL#7] 命中缓存必须校验集号：毒缓存的 episodeIndex 与当前 Emby 集不符时清掉重搜，
            // 否则一次错配（如 E8 被写成第3话）会永久生效——清除按钮和纪元洗闸都漏过 _api_ 前缀键。
            const cachedEpNum = (Number.isFinite(Number(cachedEpisodeInfo.episodeIndex)) ? Number(cachedEpisodeInfo.episodeIndex) : -1) + 1;
            if (!isSeasonCompatible(animeName, cachedEpisodeInfo.animeTitle, cachedEpisodeInfo.animeType)
                || (Number.isFinite(episode) && cachedEpNum !== episode)) {
                console.warn(`[自动匹配] 缓存集号不符(缓存第${cachedEpNum}话≠当前第${episode}话)，清除重搜`);
                window.localStorage.removeItem(unique_episode_key);
            } else if (isSeasonCompatible(animeName, cachedEpisodeInfo.animeTitle, cachedEpisodeInfo.animeType)) {
        ...
        let selectAnime_id = animaInfo.animes.findIndex(candidate => isSeasonCompatible(animeName, candidate.animeTitle, candidate.type));
        if (selectAnime_id < 0) {
        ...
        if (animeId != -1) {
            for (let index = 0; index < animaInfo.animes.length; index++) {
                if (animaInfo.animes[index].animeId == animeId
                    && isSeasonCompatible(animeName, animaInfo.animes[index].animeTitle, animaInfo.animes[index].type)) {
    ...
    function loadDanmaku(loadType = LOAD_TYPE.CHECK) {
        ...
        if (loadType === LOAD_TYPE.RELOAD) {
            // 手动匹配/显式重载拥有最高优先级：无视卡死的加载锁
        ...
        window.ede.loading = true;
                if (lsGetItem(lsKeys.useFetchPluginXml.id)) {
                    // 在线优先：先走在线匹配（含手动持久化记录），在线没出弹幕才回退服务端落盘库存。
                    // PITFALL #8: 成功分支必须放锁——ba0ee3e7 里只有 catch 分支有 finally，
                    // 服务端XML加载成功后 loading 永远为 true，切集后 loadDanmaku 被锁挡死，
                    // 所有后续剧集都显示上一集的弹幕（E8 显示 E7 的 2932 条）。
                    window.ede.onlineDanmakuOk = false;
                    getMapByEmbyItemInfo().then(() => {
                        return loadOnlineDanmaku(loadType);
                    }).then(() => {
                        const onlineCount = (window.ede.commentsParsed || []).length;
                        if (!window.ede.onlineDanmakuOk || onlineCount === 0) {
                            // 在线没出弹幕（上游挂/无匹配）→ 回退服务端库存
                            return getCommentsByPluginApi(window.ede.itemId).then((comments) => {
                                if (comments && comments.length > 0) {
    ...
    function loadOnlineDanmaku(loadType) {
        return getEpisodeInfo(loadType !== LOAD_TYPE.SEARCH)
            .then((info) => {
                return new Promise((resolve, reject) => {
                    if (!info) {
                        if (loadType !== LOAD_TYPE.INIT) {
                            return reject('播放器未完成加载');
                        } else {
                            return reject(null);
                        }
                    }
                ...
                (episodeId) => {
                    if (episodeId) {
                        if (loadType === LOAD_TYPE.RELOAD && window.ede.danmuCache[episodeId]) {
                            // PITFALL #8: return 让外层链等待弹幕真正就位（原实现不返回，调用方无法判定在线成败）
                            return createDanmaku(window.ede.danmuCache[episodeId])
                                .then(() => {
                                    console.log('弹幕就位');
                                    window.ede.onlineDanmakuOk = true;
                                })
                            ...
                            return fetchComment(episodeId).then((comments) => {
                                ...
                                if (!comments || comments.length === 0) {
                                    return null; // 在线没匹配到弹幕，等调用方回退服务端库存
                                }
        ...
        if (btnClearCache) {
            btnClearCache.addEventListener('click', () => {
                const prefixesToClear = [
                    ...
                    '_api_' // [PITFALL#7] 自动匹配结果缓存 _api_<apis>__episode_id_rel_*，此前漏清导致毒缓存永生
                ];
                lsBatchRemove(prefixesToClear);

                // 清除当前 item 的手动匹配持久化记录
                try {
                    window.localStorage.removeItem('_ede_manual_match_' + window.ede.itemId);
                    console.log('[手动匹配] 已清除当前条目的手动选择记录');
                } catch (e) {}
    ...
    function doDanmakuSwitchEpisode() {
        ...
        // 修正季偏移计算：用选项文字中的真实集号（"第7话"→7），不用下拉框位置。
        // 搜索结果常被官方按集过滤，列表不从第1话开始，位置≠集号——旧公式(位置+1-Emby集数)会写出毒偏移，
        // 导致整季自动匹配整体错位（如 E8→第3话）。
        const _epOptText = episodeNumSelect.options[episodeNumSelect.selectedIndex].text;
        const _epNumMatch = _epOptText.match(/第\s*(\d+)\s*[话話集]/) || _epOptText.match(/^\s*(\d+)\s*[-－]/);
        const _dandanEpNum = _epNumMatch ? parseInt(_epNumMatch[1], 10) : NaN;
        let seasonInfo = null;
        if (Number.isFinite(_dandanEpNum)) {
            // createSeasonInfo 内部 offset = index+1-embyEpisode；传入真实集号-1 即 offset = 弹弹集号 - Emby集号
            ...
            console.warn('[手动匹配] 无法从选项解析真实集号，跳过季偏移缓存写入（防投毒）:', _epOptText);
        }
        if (seasonInfo) {
        // 清掉同名/同animeId的旧季条目，防止脏偏移缓存共存续命（如旧的 offset=-4 导致永远跳第3话）
        try {
            const _sk = window.ede.searchDanmakuOpts._season_key;
            const oldList = JSON.parse(localStorage.getItem(_sk) || '[]');
            const pruned = oldList.filter(si => {
                try {
                    const sameAnime = String(si.animeId) === String(seasonInfo.animeId);
                    const simName = calculateStringSimilarity(si.name || '', seasonInfo.name || '') >= 0.5;
                    return !(sameAnime || simName);
                } catch (e) { return true; }
            });
            if (pruned.length !== oldList.length) {
                localStorage.setItem(_sk, JSON.stringify(pruned));
                console.log(`[手动匹配] 已清理 ${oldList.length - pruned.length} 条旧季偏移缓存`);
            }
        } catch (e) { console.warn('[手动匹配] 清理旧季缓存失败:', e); }
        ...
        // 手动选择持久化（双键写入：剧集作用域键(确定性) + itemId 键(兼容)；自动匹配最高优先读取；清除缓存按钮可移除）
        try {
            ...
            if (_episode_key) {
                localStorage.setItem('_ede_manual_match_' + _episode_key, manualPayload);
            }
            localStorage.setItem('_ede_manual_match_' + window.ede.itemId, manualPayload);
            console.log('[手动匹配] 已持久化手动选择: _ede_manual_match_{' + (_episode_key || '') + ',' + window.ede.itemId + '}');
        } catch (e) {
            console.warn('[手动匹配] 持久化失败:', e);
        }
        ...
        // 手动匹配拥有最高优先级：绕开自动匹配管道与加载锁，直接拉取所选弹幕库
        window.ede.loading = false;
        window.ede.onlineDanmakuOk = true; // 手选即真理，禁用离线兜底覆盖
        fetchComment(episodeInfo.episodeId)
            .then((comments) => {
                if (window.ede.danmaku) { try { window.ede.danmaku.destroy(); } catch (e) {} window.ede.danmaku = null; }
                window.ede.commentsParsed = [];
                window.ede.danmuCache[episodeInfo.episodeId] = comments;
                return createDanmaku(comments);
            })
            .then(() => {
                console.log(`手动匹配弹幕就位: ${window.ede.commentsParsed.length} 条`);
                window.ede.loading = false;
                const danmakuCtrEle = getById(eleIds.danmakuCtr);
                if (danmakuCtrEle && danmakuCtrEle.style.opacity !== '1') {
                    danmakuCtrEle.style.opacity = '1';
                ...
                const videoOsdDanmakuTitle = getById(eleIds.videoOsdDanmakuTitle);
                if (videoOsdDanmakuTitle) {
                    videoOsdDanmakuTitle.innerText = `弹幕：${episodeInfo.animeTitle} - ${episodeInfo.episodeTitle} - ${window.ede.commentsParsed.length}条`;
                }
            })
            .catch((err) => {
                console.error('手动匹配弹幕加载失败:', err);
                window.ede.loading = false;
                embyToast({ text: '手动匹配弹幕加载失败: ' + err });
            });
        } // end if (seasonInfo)
        closeEmbyDialog();
    }
    ...
    */
    ...
    // 缓存纪元：升级补丁后一次性清空旧自动匹配缓存（毒季偏移/错集缓存），手动记录与设置保留
    (function migrateMatchCacheEpoch() {
        const EPOCH = '4';
        try {
            if (window.localStorage.getItem('_ede_match_epoch') !== EPOCH) {
                const doomed = [];
                for (let i = 0; i < window.localStorage.length; i++) {
                    const k = window.localStorage.key(i);
                    if (k && (k.startsWith('_episode_id_rel_') || k.startsWith('_anime_season_rel_') || k.startsWith('_anime_id_rel_') || k.startsWith('_api_'))) {
                        doomed.push(k);
                    }
                }
                doomed.forEach(k => window.localStorage.removeItem(k));
                window.localStorage.setItem('_ede_match_epoch', EPOCH);
                console.log(`[缓存纪元] 已升级到 v${EPOCH}，清除 ${doomed.length} 条旧自动匹配缓存（手动记录保留）`);
            }
        } catch (e) { console.warn('[缓存纪元] 迁移失败:', e); }
    })();
